...
def PF_Jacobian(v, d, y, pq):
	# This function was written by Nathan Gray using formulas from capter 9 of
	...
	for a in range(row.shape[0]):
		...
		if i != 0 and j != 0:
			if i == j:
				# sum over n from 1:N, n~=i
				#for m in range(n):
				#	if m != i:
				#		j11[i-1, j-1] = j11[i-1, j-1] + abs(v[i]*v[m]*y[i, m])*np.sin(np.angle(y[i, m]) + d[m] - d[i])
