...
import numpy as np
from classes.power_system_cuda import PowerSystem_CUDA as PowerSystem
from numpy.linalg import inv
from numba import njit


@njit
def dyn2_f(x, vref, Pc, xp, v, slack, pv, y_gen, ws, H, Kd, Td0, Tq0, xd, xq, Ta, Tsg, Ka, Ksg, R, Vr_max, Vr_min, Efd_max, Efd_min, Psg_max, Psg_min, limits=False):
	# n = len(y) // 2 + 1
	npv = len(pv)
	ng = npv + 1
	# xp = self.xp[:npv]
	...
	vgen = np.concatenate((v[slack], (Edp + 1j * Eqp) * (1 * np.exp(1j * (th - pi / 2)))))
	Igen = np.dot(y_gen, vgen)
	v = np.abs(vgen[1:] - Igen[1:] * 1j * xp)
	Id = (Igen[pv] * np.exp(1j * (pi / 2 - th))).real
	Iq = (Igen[pv] * np.exp(1j * (pi / 2 - th))).imag
	Pe = vgen * np.conj(Igen)
	Pe = Pe.real[pv]

	# calculate x_dot
	th_dot = (w - 1) * ws
	w_dot = 1 / (2 * H) * (Pm - Pe - Kd * (w - 1))
	Eqp_dot = 1 / Td0 * (-Eqp - (xd - xp) * Id + Efd)
	Edp_dot = 1 / Tq0 * (-Edp + (xq - xp) * Iq)
	va_dot_unlim = 1 / Ta * (-Efd + Ka * (vref - v))
	Psg_dot_unlim = 1 / Tsg * (-Pm + Ksg * (Pc + 1 / R * (1 - w)))
	...
	if limits:
		for i in range(ng - 1):
			...
			if Pm[i] <= Psg_min[i] and Psg_dot_unlim[i] < 0:
				Pm_dot[i] = 0
			if Pm[i] >= Psg_max[i] and Psg_dot_unlim[i] > 0:
				Pm_dot[i] = 0
			else:
...
class DynamicSystem(PowerSystem):
	...
	def pwm_ess(self, x, vs, vs_ref, vdc_ref, vs_dot, P_A_dot, Q_A_dot):
		ng = len(self.pv) + 1
		# unpack x
		k = 1
		r_b = 0.01  # Internal battery resistance
		c_dc = 0.1  # Capacitor on DC side of converter
		...
		kpss_m = 0.05414
		Tw_m = 5
		uw_m_dot = 1 / Tw_m * (Q_A_dot * Tw_m * kpss_m - uw_m)
		...
		Tw_f = 5
		kpss_f = 0.0001898
		uw_f_dot = 1 / Tw_f * (P_A_dot * Tw_f * kpss_f - uw_f)
		#  - Active Compensator
		T1_f = 3.309
		T2_f = 0.4
		T3_f = 3.309
		...
		v_dc_dot = round(1 / c_dc * (-Idc1 + Idc2), 10)
		m_dot = 1 * (-0.1 * vs_dot + 0.85 * (vs_ref - vs))  # + upss_m_dot
		phi_dot = 1 * (-0.2 * v_dc_dot + 2 * (vdc_ref - v_dc)) + upss_f_dot

		Ps = v_dc * Idc1

		return np.array([m_dot, phi_dot, v_dc_dot, uw_m_dot, u1_m_dot, upss_m_dot, uw_f_dot, u1_f_dot, upss_f_dot])

	def pam_ess(self, x, vs, vs_ref, vdc_ref, vs_dot, v_dc_dd, P_A_dot):
		ng = len(self.pv) + 1
		# unpack x
		k = 1
		r_b = 0.01  # Internal battery resistance
		...
		# lead compensator
		k1 = 5  # 40
		wmax = 6  # omega where phase angle is maximum
		wmag = 5 * pi / 2  # maximum phase shift
		b = 1 / wmag ** 2
		...
		Tw_pam = 5
		kpss_pam = 0.05951
		uw_pam_dot = 1 / Tw_pam * (P_A_dot * Tw_pam * kpss_pam - uw_pam)
		#  - Active Compensator
		# T1_pam = t2#1.328
		# T2_pam = t1#0.4
		# T3_pam = t2#1.328
		...
		T3_pam = T1_pam
		T4_pam = T2_pam
		u1_pam_dot = 1 / T1_pam * (uw_pam + uw_pam_dot * T2_pam - u1_pam)
		upss_pam_dot = 1 / T3_pam * (u1_pam + u1_pam_dot * T4_pam - upss_pam)
		#
		# upss_pam_dot = 0
		# upss_pam = 0
		v_dc_dot = 1 / c_dc * (-Idc1 + Idc2)
		m_dot = 0
		phi_dot = 0.1 * (1 / k * (-vs_dot + upss_pam_dot) - v_dc_dd) + 0.5 * (
					1 / k * (vs_ref - vs + upss_pam) - v_dc_dot)

		# Ps = v_dc * Idc1

		return np.array([m_dot, phi_dot, v_dc_dot, 0, 0, 0, uw_pam_dot, u1_pam_dot, upss_pam_dot])
	...
	def dyn1pwm_f(self, x, y, vref, Pc, vs_ref, vdc_ref, vs_dot, P_A_dot, Q_A_dot, limits=False):
		n = len(y) // 2 + 1
		npv = len(self.pv)
		ng = npv + 1
		xdp = self.xdp[:npv]
		...
		# calculate x_dot
		vs = v[7]
		x_ess_dot = self.pwm_ess(x_ess, vs, vs_ref, vdc_ref, vs_dot, P_A_dot, Q_A_dot)
		th_dot = (w - 1) * self.ws
		w_dot = 1 / (2 * self.H) * (Pm - Pg - self.Kd * (w - 1))  # Pg = vd * Id + vq * Iq
	...
	def dyn1pam_f(self, x, y, vref, Pc, vs_ref, vdc_ref, vs_dot, P_A_dot, vdc_dd, limits=False):
		n = len(y) // 2 + 1
		npv = len(self.pv)
		ng = npv + 1
		xdp = self.xdp[:npv]
		...
		# calculate x_dot
		vs = v[7]
		x_ess_dot = self.pam_ess(x_ess, vs, vs_ref, vdc_ref, vs_dot, vdc_dd, P_A_dot)
		th_dot = (w - 1) * self.ws
		w_dot = 1 / (2 * self.H) * (Pm - Pg - self.Kd * (w - 1))  # Pg = vd * Id + vq * Iq
		Eqp_dot = 1 / self.Td0 * (-Eqp - (self.xd - xdp) * Id + Efd)
		Edp_dot = 1 / self.Tq0 * (-Edp + (self.xq - xqp) * Iq)
	...
	def dyn2_f(self, x, vref, Pc, limits=False):
		return dyn2_f(x, vref, Pc, self.xp, self.v, self.slack, self.pv, self.y_gen, self.ws, self.H, self.Kd, self.Td0,
			   self.Tq0, self.xd, self.xq, self.Ta, self.Tsg, self.Ka, self.Ksg, self.R, self.Vr_max, self.Vr_min,
			   self.Efd_max, self.Efd_min, self.Psg_max, self.Psg_min, limits=limits)
		# # n = len(y) // 2 + 1
		# npv = len(self.pv)
		# ng = npv + 1
		# xp = self.xp[:npv]
