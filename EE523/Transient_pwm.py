...
if __name__ == "__main__":
	...
	def f_ess(x_states, y_input, vs_dot, P_A_dot, Q_A_dot):
		return dps.dyn1pwm_f(x_states, y_input, type1['vref'], type1['Pc'], vs_ref, vdc_ref, vs_dot, P_A_dot, Q_A_dot, limits=False)
	...
	vs = np.array([v[7], v[7]])
	Ps, Qs = dps.ess_out(x_ess[-n_ess:], vs_ess[-1])
	t_plot = 0
	t = 0
	h = 0.001
	...
	while t < t_end:
		...
		xk_ess = np.c_[xk_ess, xk_ess[:, -1] + h * f_ess(xk_ess[:, -1], yf_ess, vs_dot, P_A_dot, Q_A_dot)]
	...
	ax4.set_title('Tie line Active Power Response (PWM)')
	ax4.legend()
	plt.show()
	# fig4.savefig("ActiveResponse_PWM.png")
	fig4 = plt.figure()
	ax4 = fig4.add_subplot(111)
	...
	ax4.set_ylabel('Power (p.u.)')
	ax4.set_xlabel('time (ms)')
	ax4.set_title('Tie line Reactive Power Response (PWM)')
	ax4.legend()
	plt.show()
	# fig4.savefig("ReactiveResponse_PWM.png")
	fig4 = plt.figure()
	ax4 = fig4.add_subplot(111)
	...
	ax4.set_xlabel('time (ms)')
	ax4.set_title('Tie line Voltage Response (PWM)')
	ax4.legend()
	plt.show()
	# fig4.savefig("VoltageResponse_PWM.png")
	fig4 = plt.figure()
	ax4 = fig4.add_subplot(111)
	...
	ax4.set_ylabel('Power (p.u.)')
	ax4.set_xlabel('time (ms)')
	ax4.set_title('Stabilizing Response (PWM)')
	ax4.legend()
	plt.show()
	# fig4.savefig("StabilizerResponse_PWM.png")
